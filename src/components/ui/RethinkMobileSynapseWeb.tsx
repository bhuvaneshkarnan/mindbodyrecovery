...
export const RethinkMobileSynapseWeb: React.FC<{ className?: string }> = ({ className = "" }) => {
  // Vibrant high-contrast somatic palette for light background
  const strokeColor = "#8C5B41"; // Rich Somatic Terracotta
  const glowColor = "#C79A45";   // Warm Gold Glow
  const faintColor = "rgba(140, 91, 65, 0.35)";

  // 7 Primary Curly Connecting Axons to Therapy Nodes in mobile 2-col constellation
  const AXON_PATHS = [
    {
    ...
    },
    {
      ...
      d: generateCurlyAxon(180, 650, 180, 790, 16, 3),
      endX: 180,
      endY: 790,
      dur: "2.8s",
      delay: "1.2s",
  ...
  // Uneven secondary dendritic branches & lateral inter-synaptic bridges
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id="mob-rethink-soma-halo-v3" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="#C79A45" stopOpacity="0.25" />
      ...
      <circle cx="180" cy="110" r="120" fill="url(#mob-rethink-soma-halo-v3)" />
      ...
      <circle r="4.5" fill={glowColor} opacity="0.95">
        <animateMotion
          dur="4.2s"
          repeatCount="indefinite"
          path={centralSpine}
      ...
      {/* 4. CURLY AXON PATHS CONNECTING SOMA TO ALL 7 THERAPY NODES */}
      {AXON_PATHS.map((axon) => (
        <g key={axon.id}>
          ...
          {/* Action Potential Traveling Spark (Terracotta body + Gold core) */}
          <circle r="4.5" fill={glowColor} opacity="0.95">
            <animateMotion
              dur={axon.dur}
              repeatCount="indefinite"
